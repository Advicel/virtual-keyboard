...
class Keyboard {
    ...
    init() {
        ...
        this.elements.info.innerHTML = "Переключение раскладки: ALT+SHIFT <br> Работы выполнена на ОС Windows";
        ...
        this.elements.wrapper.append(this.elements.textarea,this.elements.keyboard,this.elements.info);
...
onBackspace(){
    ...
    this.elements.textarea.value = value.slice(0, newSelectionStart)+value.slice(selectionStart);

    this.setCursorPosition(newSelectionStart);
}
onDelete(){
    const{ value,selectionStart} = this.elements.textarea;
    this.elements.textarea.value = value.slice(0, selectionStart)+value.slice(selectionStart+1);
